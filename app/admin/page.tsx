...
import { adminEmail, hasAdminSession } from "../../src/lib/admin-auth";
...
export default async function Admin() {
  ...
  const db = getDb(); if (!db) return <AdminMessage title="Banco não configurado." text="Defina DATABASE_URL no ambiente do servidor." />;
  const result = await db.query("select r.id,r.protocol,r.status,r.shirt_size,r.submitted_at,p.full_name,p.phone_e164,e.location from public.registrations r join public.users u on u.id=r.user_id left join public.profiles p on p.id=r.user_id left join public.events e on e.id=r.event_id order by r.created_at desc");
  const rows = result.rows.map((r) => ({ id: r.id, protocol: r.protocol, status: r.status, shirt_size: r.shirt_size, submitted_at: r.submitted_at, profiles: r.full_name ? { full_name: r.full_name, phone_e164: r.phone_e164 } : null, events: r.location ? { location: r.location } : null })) as Registration[];
  ...
  return <main className="admin-shell"><div className="container">
    <header className="account-head"><Link href="/" className="brand"><span className="brand-mark">MF</span> Painel administrativo</Link><div className="admin-head-actions"><span className="admin-user">{adminEmail}</span><form action="/api/admin/signout" method="post"><button className="button button-secondary" type="submit">Sair</button></form></div></header>
