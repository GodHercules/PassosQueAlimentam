  ...
  return (
    <>
      ...
      <main className="road-site">
        ...
        <section className="supporters-section" id="apoiadores" aria-labelledby="supporters-title"><div className="container supporters-intro"><div><span className="stop-label">QUEM CAMINHA COM A GENTE</span><h2 id="supporters-title">Essa rota só existe<br /><em>porque vocês apoiam.</em></h2></div><p>A cada parceiro, uma nova possibilidade. Obrigado por transformar solidariedade em movimento.</p></div><div className="supporter-marquee" aria-label="Apoiadores da corrida"><div className="supporter-track"><div className="supporter-set">{["APOIO", "COMUNIDADE", "MOVIMENTO", "CUIDADO", "PARCERIA", "FUTURO", "SOLIDARIEDADE", "ENERGIA", "IMPACTO", "JUNTOS"].map((name, index) => <div className="supporter-logo" key={`a-${name}`}><span>{String(index + 1).padStart(2, "0")}</span><strong>{name}</strong></div>)}</div><div className="supporter-set" aria-hidden="true">{["APOIO", "COMUNIDADE", "MOVIMENTO", "CUIDADO", "PARCERIA", "FUTURO", "SOLIDARIEDADE", "ENERGIA", "IMPACTO", "JUNTOS"].map((name, index) => <div className="supporter-logo" key={`b-${name}`}><span>{String(index + 1).padStart(2, "0")}</span><strong>{name}</strong></div>)}</div><div className="supporter-set" aria-hidden="true">{["APOIO", "COMUNIDADE", "MOVIMENTO", "CUIDADO", "PARCERIA", "FUTURO", "SOLIDARIEDADE", "ENERGIA", "IMPACTO", "JUNTOS"].map((name, index) => <div className="supporter-logo" key={`c-${name}`}><span>{String(index + 1).padStart(2, "0")}</span><strong>{name}</strong></div>)}</div><div className="supporter-set" aria-hidden="true">{["APOIO", "COMUNIDADE", "MOVIMENTO", "CUIDADO", "PARCERIA", "FUTURO", "SOLIDARIEDADE", "ENERGIA", "IMPACTO", "JUNTOS"].map((name, index) => <div className="supporter-logo" key={`d-${name}`}><span>{String(index + 1).padStart(2, "0")}</span><strong>{name}</strong></div>)}</div></div></div><div className="supporter-note">Espaços reservados para as marcas que apoiam a ação <span>●</span></div></section>
